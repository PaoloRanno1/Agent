...
Original file is located at
    https://colab.research.google.com/drive/1dsEwsauzAPARmGkg6BYlG9Wq-jm9j9lq
"""
...
class PDFUploadTool(BaseTool):
    ...
    def _run(self, file_path: str, file_name: Optional[str] = None) -> str:
        return self.pdf_manager.upload_pdf(file_path, file_name)
...
class PDFSummaryTool(BaseTool):
    ...
    def _run(self, file_name: Optional[str] = None) -> str:
        pdf_path = self.pdf_manager.get_pdf_path(file_name)

        if pdf_path is None:
            if not self.pdf_manager.list_pdfs():
                return "❌ No PDF files have been uploaded yet."
            else:
                return f"❌ PDF file '{file_name}' not found."

        try:
            summary = summarize_legal_pdf(pdf_path, self.openai_api_key)
